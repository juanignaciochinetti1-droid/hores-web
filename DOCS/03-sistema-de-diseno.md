...
(`catalogo_templates.xml`), con selector `.oe_structure .clase` para tener
suficiente especificidad sobre el CSS base de Odoo. Antes del 26/08/2026
era `#wrap.oe_structure .clase` (más específico), pero `site_footer` pasó
a vivir en la región `#footer` de Odoo en vez de dentro de `#wrap` (ver
[arquitectura](02-arquitectura-proyecto.md#footer-único-en-todo-el-sitio))
— `#wrap` y `#footer` comparten la clase `oe_structure`, así que aflojar
el selector a solo `.oe_structure` cubre las dos regiones sin duplicar
reglas.
...
| `.hc-cart-badge` | Numerito del carrito sobre el ícono 🛒 del header, se actualiza por JS al agregar un producto sin recargar la página | `site_header`, en todas las páginas — ver [pedidos](07-pedidos.md) |
